...
function Validation(){
   ...
   *********************************************************************/
   var minMaxRange=function(min,max){
      ...
      if(!!min&&!!max)
         return " pomiêdzy "+min+" i "+max;
      else if(!!min)
         return " wiêksze lub równe do "+min;
      else if(!!max)
         return " mniejsze lub równe ni¿ "+max;
      else return "";
   }
   /*********************************************************************
   Function: dateOrTime
   ...
   ******************************************************************************/
   var isValidElement=function(element,event){
      ...
      if(propertyOn(getProperty(element,"REQUIRED")) && !sValue){
         Validation.Err.raise(element, "WprowadŸ wartoœæ", "REQUIRED",event);
         return false;
      }
      // FLOAT, NUMBER
      if(((bFirst=propertyOn(getProperty(element,"FLOAT")))||propertyOn(getProperty(element,"NUMBER"))) && sValue){
         if(!new RegExp("^"+(propertyOn(bSigned)?"-?":"")+"(\\d*(,?\\d{3})*\\.?\\d+|\\d+(,?\\d{3})*\\.?\\d*)$").test(sValue))
            ...
            pass=false;
         if(!pass){
            Validation.Err.raise(element, "WprowadŸ "+(bSigned?"":"dodatni¹ ")+"liczbê"+minMaxRange(formatNumber(iMin),formatNumber(iMax)),bFirst?"FLOAT":"NUMBER",event);
            return false;
         }
      }
      // AMOUNT, CURRENCY
         ...
         if(!new RegExp("^("+
            ...
            pass=false;
         if(!pass){
            Validation.Err.raise(element, "WprowadŸ "+(bSigned?"":"dodatni¹ ")+"kwotê"+minMaxRange(formatNumber(iMin),formatNumber(iMax)),bFirst?"AMOUNT":"CURRENCY",event);
            return false;
         }
      }
      // INTEGER
         ...
         if (!new RegExp("^"+((bSigned)?"-?":"")+"(\\d|\\d{2}|\\d{3})(,?\\d{3})*$").test(sValue))
            ...
            pass=false;
         if(!pass){
            Validation.Err.raise(element, "WprowadŸ "+(bSigned?"n ":"dodatni¹ ")+"liczbê ca³kowit¹"+minMaxRange(formatNumber(iMin),formatNumber(iMax)),"INTEGER",event);
            return false;
         }
      }
      // DATE, DATETIME
         ...
         if(typeof (date=toDate(sValue,format))=="undefined")
            ...
            pass=false;
         if(!pass){
            Validation.Err.raise(element,"WprowadŸ "+dateOrTime(format)+" wartoœæ"+minMaxRange(iMin,iMax)+" wed³ug wzorca:\n\t"+format.replace(/ap/i,"AM/PM").toUpperCase(),bFirst?"DATE":"DATETIME",event);
            return false;
         }
      }
      // PHONE
         ...
         if(!(iDigits==10||iDigits==11&&/^1/.test(sPhone))){
            Validation.Err.raise(element,"WprowadŸ poprawny numer telefonu","PHONE",event);
            return false;
         }
      }
      // EMAIL
         ...
         if(!/^[\w_-]+(\.[\w_-]+)*@[\w_-]+(\.[\w_-]+)*\.[a-z]{2,4}$/i.test(sValue)){
            Validation.Err.raise(element,"WprowadŸ poprawny adres poczty elektronicznej","EMAIL",event);
            return false;
         }
      }
      // ZIP Code
         ...
         if(!/^\d{5}(-?\d{4})?$/.test(sValue)){
            Validation.Err.raise(element, "WprowadŸ poprawny kod pocztowy","ZIP",event);
            return false;
         }
      }
      // REGEXP
      if(propertyOn(oRegexp=getProperty(element,"REGEXP"))&&sValue){
         ...
         if(!oRegexp.test(sValue)){
            Validation.Err.raise(element, "WprowadŸ poprawn¹ wartoœæ", "REGEXP",event);
            return false;
         }
      }
      // MAXLENGTH
      if(sValue&&(iLength=getProperty(element,"maxLength"))&&!/\D/.test(iLength)&&sValue.length>iLength){
         Validation.Err.raise(element,"WprowadŸ wartoœæ o d³ugoœci nie wiêkszej ni¿ " + formatNumber(iLength) + " znaków", "MAXLENGTH",event);
         return false;
      }
         ...
         for(var oNewElement,i=0,iFields=vAnd.length; i<iFields; i++){
            if((oNewElement=(typeof vAnd[i].form=="object")?vAnd[i]:element.form.elements[trim(vAnd[i])])){
               if(!!getValueOf(oNewElement)){
                  Validation.Err.raise(element, "WprowadŸ wartoœæ", "AND",event);
                  return false;
               }
            }
         }
      ...
      if((or=getProperty(element,"OR"))&&!sValue){
         ...
         if(fields){
            ...
            if(!bValue){
               Validation.Err.raise(element, message?message:"WprowadŸ wartoœæ", "OR",event);
               return false;
            }
         }
      }
            ...
            if(!element._){
               ...
               element.onkeypress=function(oEvent){ //NN passes event object
                  ...
                  if(propertyOn(filter)){
                     ...
                     if((propertyOn(getProperty(this,"FLOAT"))||propertyOn(getProperty(this,"NUMBER"))) && (keyChar==",")){
                        oEvent.keyCode=46;
                        keyChar=".";
                     }
