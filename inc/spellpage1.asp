...
function replacehtml(){
  ...
  asourcetextelement=window.dialogArguments;
  document.all('lastclickedword').value='';
  aimgelementscount=0;
  aframedocument=document.all("myspellframe").contentWindow.document;  
  adocumentbody=aframedocument.body;    

  if (asourcetextelement.nodeName=='IFRAME') {
     adocumentbody.innerHTML=asourcetextelement.contentWindow.document.body.innerHTML;
  } else {
     adocumentbody.innerHTML=asourcetextelement.value.replace(/\t/gi,'&nbsp;&nbsp;&nbsp;&nbsp;').replace(/\r\n/gi,'<br>').replace(/  /gi,'&nbsp;&nbsp;');
  }
...
//  xml_dom.loadXML('<?xml version="1.0" encoding="utf-8" ?> <root/>');
//  xml_dom.documentElement.setAttribute("xmlns:dt", "urn:schemas-microsoft-com:datatypes");
//  var l_node1 = xml_dom.createElement("file1");
//  l_node1.dataType = "bin.base64";
//  l_node1.nodeTypedValue = atextrange.text;
...
//  xmlHttp.send('<?xml version="1.0" encoding="utf-8" ?> <DANE mode="check"><![CDATA[' + atextrange.text + ']]></DANE>');
...
//  sendtext='<DANE mode="check"><![CDATA[' + atext + ']]></DANE>';
  window.setTimeout(function (){ xmlHttp.send('<?xml version="1.0" encoding="utf-8" ?> <DANE mode="check"><![CDATA[' + atextrange.text + ']]></DANE>')});  
}

function replaceHTMLStep2(xmlHttp,atextrange) {
//  alert(xmlHttp.responseText.substr(1900,100000))
//  alert(xmlHttp.responseText)
  ...
  for(i=0; i<nlines; i++){
     ...
     for(j=0; j<misstakes.length; j++){
        ...
        if (nsugestions==0){
           ...
           aretdiv[aretdivpos++]="<div onmouseover='this.style.background=\"#ffffff\"' onmouseout='this.style.background=\"wheat\"' STYLE='font-family:verdana;FONT-SIZE: 8pt; height:20px; color:red; background:wheat; border:1px solid black; padding:3px; padding-left:10px; cursor:hand;' onclick='parent.processPopup(\""+original+"\");' id='div_sug_"+wordoffset+"' name='div_sug_"+wordoffset+"'>"+(k+1)+') '+original+"</div>";
        }   
...
function doCorrectOK(){
  ...
  aframedocument=document.all('myspellframe').contentWindow.document;
  if (asourcetextelement.nodeName=='IFRAME'){
     asourcetextelement.contentWindow.document.body.innerHTML=aframedocument.body.innerHTML;
  } else {
     asourcetextelement.value=aframedocument.body.innerText;
  }   
  window.returnValue=asourcetextelement;
  window.close();
}
function doAdd(aword){
  ...
  sendtext='<DANE mode="add"><![CDATA[' + aword.toLowerCase() + ']]></DANE>';
              ...
              <table border=0 height=100% width=100% cellpadding=5 cellspacing=0>     
                ...
                <tr>
                   ...
                   <td align=left width=50%>
                      <button onclick='javascript:window.close()'>ANULUJ</button>
                   </td>
                </tr>
                <tr>
