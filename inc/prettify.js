...
(function () {
  ...
  var SQL_KEYWORDS = (
"add exists primary all " +
"exit print alter external proc " +
"and fetch procedure any file " +
"public as fillfactor raiserror asc " +
"for read authorization foreign readtext " +
"backup freetext reconfigure begin freetexttable " +
"references between from replication break " +
"full restore browse function restrict " +
"bulk goto return by grant " +
"revert cascade group revoke case " +
"having right check holdlock rollback " +
"checkpoint identity rowcount close identity_insert " +
"rowguidcol clustered identitycol rule coalesce " +
"if save collate in schema " +
"column indexin securityaudit commit innerindex " +
"select compute insertinner session_user constraint " +
"intersectinsert set contains intointersect setuser " +
"containstable isinto shutdown continue joinis " +
"some convert keyjoin statistics create " +
"killkey system_user cross leftkill table " +
"current likeleft tablesample current_date linenolike " +
"textsize current_time loadlineno then current_timestamp " +
"national load to current_user nochecknational " +
"top cursor nonclusterednocheck tran database " +
"notnonclustered transaction dbcc nullnot trigger " +
"deallocate nullifnull truncate declare ofnullif " +
"tsequal default offof union delete " +
"offsetsoff unique deny onoffsets unpivot " +
"desc openon update disk opendatasourceopen " +
"updatetext distinct openqueryopendatasource use distributed " +
"openrowsetopenquery user double openxmlopenrowset values " +
"drop optionopenxml varying dump oroption " +
"view else orderor waitfor end " +
"outerorder when errlvl overouter where " +
"escape percentover while except pivotpercent " +
"with exec planpivot writetext execute " +
"plan precision " +
//MDX Extensions
"absolute desc leaves self_before_after " +
"actionparameterset descendants level session addcalculatedmembers " +
"description levels set after dimension " +
"linkmember settoarray aggregate dimensions linregintercept " +
"settostr all distinct linregpoint sort " +
"allmembers distinctcount linregr2 stddev ancestor " +
"drilldownlevel linregslope stddevp ancestors drilldownlevelbottom " +
"linregvariance stdev and drilldownleveltop lookupcube " +
"stdevp as drilldownmember max storage " +
"asc drilldownmemberbottom measure stripcalculatedmembers ascendants " +
"drilldownmembertop median strtomember average drilluplevel " +
"member strtoset axis drillupmember members " +
"strtotuple basc drop membertostr strtoval " +
"bdesc empty min strtovalue before " +
"end mtd subset before_and_after error " +
"name sum bottomcount except nametoset " +
"tail bottompercent excludeempty nest this " +
"bottomsum extract nextmember toggledrillstate by " +
"false no_allocation topcount cache filter " +
"no_properties toppercent calculate firstchild non " +
"topsum calculation firstsibling nonemptycrossjoin totals " +
"calculationcurrentpass for not_related_to_facts tree calculationpassvalue " +
"freeze null true calculations from " +
"on tupletostr call generate openingperiod " +
"type cell global or union " +
"cellformulasetlist group pages unique chapters " +
"grouping parallelperiod uniquename children head " +
"parent update clear hidden pass " +
"use closingperiod hierarchize periodstodate use_equal_allocation " +
"coalesceempty hierarchy post use_weighted_allocation column " +
"ignore predict use_weighted_increment columns iif " +
"prevmember username correlation includeempty properties " +
"validmeasure count index property value " +
"cousin intersect qtd var covariance " +
"is rank variance covariancen isancestor " +
"recursive variancep create isempty relative " +
"varp createpropertyset isgeneration rollupchildren visual " +
"createvirtualdimension isleaf root visualtotals crossjoin " +
"issibling rows where cube item " +
"scope with current lag sections " +
"wtd currentcube lastchild select xor " +
"currentmember lastperiods self ytd default_member " +
"lastsibling self_and_after defaultmember lead self_and_before " +
//SQL Shape
"shape "
  );
  ...
  for (var k = 0; k < KEYWORDS.length; k++) {
    ...
    for (var i = 0; i < kw.length; i++) {
      if (kw[i]) {
         ...
         PR_keywords[kw[i].toUpperCase()] = true;
      }
    }
  }
}).call(this);
